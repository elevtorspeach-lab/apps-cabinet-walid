self.addEventListener('message', async (event)=>{
  ...
  try{
    ...
    if(type === 'xlsx-styled-multi-sheet-export'){
      ...
      if(typeof ExcelJS === 'undefined') throw new Error('ExcelJS library unavailable in worker.');
      const sheets = Array.isArray(data.sheets) ? data.sheets : [];
      const workbook = new ExcelJS.Workbook();
      const headerImageDataUrl = String(data.headerImageDataUrl || '').trim();
      const imageId = headerImageDataUrl ? workbook.addImage({ base64: headerImageDataUrl, extension: 'jpeg' }) : null;
      const border = {
      ...
      sheets.forEach((entry, index)=>{
        ...
        sheet.getCell('A6').value = {
          richText: [
            ...(subtitle ? [{ text: subtitle, font: { name: 'Arial', size: 16, bold: true, color: { argb: 'FF1A4590' } } }] : []),
            ...(subtitle ? [{ text: '   ', font: { name: 'Arial', size: 11, color: { argb: 'FF111111' } } }] : []),
            { text: editionLabel, font: { name: 'Arial', size: 11, bold: true, color: { argb: 'FF111111' } } }
          ]
        };
      ...
      self.postMessage({ type: 'xlsx-styled-multi-sheet-export-result', requestId, ok: true, buffer }, [buffer]);
      return;
    }
    ...
    if(type === 'xlsx-multi-sheet-export'){
      ...
      sheets.forEach((sheet, index)=>{
        const sheetAoa = Array.isArray(sheet?.aoa) ? sheet.aoa : [];
        const ws = self.XLSX.utils.aoa_to_sheet(sheetAoa);
        const sheetColWidths = Array.isArray(sheet?.colWidths) ? sheet.colWidths : [];
        if(sheetColWidths.length) ws['!cols'] = sheetColWidths;
