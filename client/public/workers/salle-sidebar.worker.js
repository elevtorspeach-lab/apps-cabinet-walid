...
function matchesJudgeKey(candidateKey, targetKey) {
...
self.addEventListener('message', (event) => {
  ...
  const renderLimit = Number.isFinite(Number(payload.renderLimit))
    ? Math.max(1, Number(payload.renderLimit))
  ...
  audienceRows.forEach((row) => {
    ...
    row.judgeKeys.forEach((candidateKeyRaw) => {
      ...
      if (!matchedKeys) {
        matchedKeys = targetJudgeKeys.filter((targetJudgeKey) => matchesJudgeKey(candidateKey, targetJudgeKey));
