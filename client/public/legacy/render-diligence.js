...
function buildDiligenceCountLabel(totalRows){
  ...
  const useDelegationFilter = typeof shouldUseDiligenceDelegationFilter === 'function'
    ? shouldUseDiligenceDelegationFilter(filterDiligenceProcedure)
    : true;
  ...
  if(useDelegationFilter || isSaisieArret){
...
function shouldShowDiligenceNantissementColumns(rows){
...
function getDiligenceColCount(){
  if(diligenceVirtualCompactProcedureMode === 'scitf') return 20;
  ...
  if(shouldShowDiligenceNantissementColumns(diligenceVirtualRows)){
    if(hasDiligenceNantissementNbRows(diligenceVirtualRows)) return 32;
  ...
  if(diligenceVirtualShowAssColumns){
    ...
    if(curateurExtraCols) return 20 + curateurExtraCols;
...
function buildDiligenceHeadHtml(){
  ...
  if(diligenceVirtualShowCommandementColumns){
    ...
    return `
      ...
      <th>Date dépôt</th>
      <th>Execution N°</th>
      <th>PLIE / DELEGATION</th>
      <th>Pub au journal</th>
      <th>Notif Conservateur</th>
      <th>Notif débiteur</th>
      ${cmdExpanded ? '<th>Lettre Rec</th><th>Curateur N°</th><th>ORD</th><th>SORT ORD</th><th>Notif curateur</th><th>Sort notif</th><th>Avis curateur</th><th>PV Police</th>' : ''}
      <th>Ref expertise</th>
      <th>Ord</th>
      <th>Expert</th>
      <th>Mise a prix</th>
  ...
  if(shouldShowDiligenceNantissementColumns(diligenceVirtualRows)){
    ...
    return `
      ...
      <th>Observation</th>
      ${showNbFollowupColumns ? '<th>Lettre Rec</th><th>Curateur N&deg;</th><th>ORD</th><th>SORT ORD</th><th>Notif curateur</th><th>Avis curateur</th><th>PV Police</th>' : ''}
  ...
  return `
    ...
    <th>Date dépôt</th>
    <th>Référence dossier</th>
    ${diligenceVirtualShowAssColumns ? '<th>Juge</th>' : ''}
    <th>${diligenceVirtualShowAssColumns || compactMode === 'injonction' || compactMode === 'sbien' || compactMode === 'sfdc' ? 'Sort ORD' : 'Ordonnance'}</th>
    ${showSharedNotificationColumns ? `<th>Notification N°</th>${diligenceVirtualShowAssColumns ? '<th>PLIE / DELEGATION</th>' : ''}<th>Sort notification</th><th>Observation</th>` : ''}
    ${showAssFollowupColumns ? '<th>Lettre Rec</th><th>Curateur N°</th><th>ORD</th><th>SORT ORD</th><th>Notif curateur</th><th>Sort notif</th><th>Avis curateur</th><th>PV Police</th>' : ''}
...
function renderDiligencePlainTextCell(row, procEncoded, field, value){
...
function renderDiligenceRowHtml(row, showPlieColumn){
  ...
  if(diligenceVirtualCompactProcedureMode === 'scitf' && isDiligenceSciTfProcedure(row?.procedure)){
    return `
      <tr ${rowAttrs}>
        ...
        <td>${escapeHtml(row.dossier?.referenceClient || '-')}</td>
        <td>${renderDiligenceEditableCell(row, procEncoded, 'cinNonDebiteur', row.dossier?.cinNonDebiteur || '')}</td>
        <td>${renderDiligenceEditableCell(row, procEncoded, 'debiteur', row.dossier?.debiteur || '')}</td>
        <td>${renderDiligenceEditableCell(row, procEncoded, 'caution', row.dossier?.caution || '')}</td>
        ...
        <td class="diligence-sci-tf-observation-cell">${renderDiligenceEditableCell(row, procEncoded, 'observation', row.details?.observation || '')}</td>
        ...
        <td>${renderDiligenceEditableCell(row, procEncoded, 'adresse', row.details?.adresse || row.dossier?.adresse || '')}</td>
        ...
        <td>${renderDiligenceEditableCell(row, procEncoded, 'type', row.dossier?.type || '')}</td>
      </tr>
    `;
  }
  ...
  if(shouldShowDiligenceNantissementColumns(diligenceVirtualRows) && isDiligenceNantissementProcedure(row?.procedure)){
    ...
    const nbFollowupCells = showNbFollowupColumns
      ? (isAssNbLayoutValue
        ? `
          ...
          <td>${renderDiligencePlainTextCell(row, procEncoded, 'ord', row.details?.ord || '')}</td>
    ...
    return `
      <tr ${rowAttrs}>
        ...
        <td class="diligence-nantissement-observation-cell">${renderDiligenceEditableCell(row, procEncoded, 'observation', row.details?.observation || '')}</td>
        ${nbFollowupCells}
        <td>${renderDiligenceEditableCell(row, procEncoded, 'certificatNonAppelStatus', certificatNonAppelValue)}</td>
        <td>${renderDiligenceEditableCell(row, procEncoded, 'executionNo', executionValue)}</td>
        ...
        <td>${renderDiligenceEditableCell(row, procEncoded, 'historique', row.details?.historique || '')}</td>
        <td>${renderDiligenceEditableCell(row, procEncoded, 'tribunal', tribunalValue)}</td>
        <td>${renderDiligenceEditableCell(row, procEncoded, 'boiteNo', row.dossier?.boiteNo || '')}</td>
      </tr>
    `;
  ...
  const notificationCells = showSharedNotificationColumns
    ? `
      ...
      <td>${renderDiligenceEditableCell(row, procEncoded, notificationSortField, notificationSortValue)}</td>
      <td class="${showCompactInjonctionColumns ? 'diligence-injonction-observation-cell' : (isDiligenceAssProcedure(row?.procedure) ? 'diligence-ass-observation-cell' : (isDiligenceNantissementProcedure(row?.procedure) ? 'diligence-nantissement-observation-cell' : ''))}">${renderDiligenceEditableCell(row, procEncoded, 'observation', row.details?.observation || '')}</td>
    `
    : '';
  ...
  const nbFollowupCells = isAssExpandedLayout
    ? `
      ...
      <td>${hideWrap(renderDiligencePlainTextCell(row, procEncoded, 'ord', row.details?.ord || ''))}</td>
    ...
    `
    : `${showAssFollowupColumns ? '<td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td>' : ''}${curateurStandaloneCells}`;
  const standardCells = `
...
function renderDiligence(options = {}){
  ...
  const finalizeDiligenceRender = (rows)=>{
    try{
      ...
      diligenceVirtualShowAssColumns = shouldShowDiligenceAssColumns(pageData.rows);
  ...
  if(diligenceQuery && allRows.length >= 1200 && !!getDiligenceFilterWorker()){
    ...
    const narrowedRows = allRows.filter(row=>{
      ...
      if(
        ...
        && (typeof shouldUseDiligenceDelegationFilter !== 'function' || shouldUseDiligenceDelegationFilter(row?.procedure))
